 ...
 * @version     $Id: back_in_stock_notifications_functions.php 2022-08-16 torvista
 */
 ...
 *
 * @author  Conor Kerr <zen-cart.back-in-stock-notifications@dev.ceon.net>
 * @param   integer   $languages_id   Language id to select email sending language and constants
 * @param   boolean   $test_mode   Flag to indicate if e-mails should actually be sent or just a sample e-mail generated for the admin for test purposes.
 * @param   boolean   $delete_subscriptions   Override for using the real sending option, but repeatedly for testing
...
function sendBackInStockNotifications($test_mode = false)
{
	...
	while (!$email_addresses_result->EOF) {
		...
		$customer_name = (!is_null($email_addresses_result->fields['customers_firstname']) ?
			...
			$email_addresses_result->fields['name']);//name entered in BISN form by subscriber/not a registered customer
