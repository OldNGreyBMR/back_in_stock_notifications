 ...
 */
// BMH  2021-10-10  back in stock notifications line 271; change to include model
//					quality group by in selects for MySQL 5.7; 
//                  line 273 mysqli_affected_rows function call {{{ sendBackInStockNotifications()
//BMH   2022-02-08	line 60 include all fields in group by clause
//      2022-08-26  ln 307 sendBackInStockNotificationEmail define global variables for email
//                  ln 129 add model
// 
...
function sendBackInStockNotifications($test_mode = false)
{
	...
	$email_addresses_query_raw = "
		...
		GROUP BY
			email_address, customers_email_address ,bisns.name, c.customers_firstname, c.customers_lastname
		ORDER BY
		...
		if (!array_key_exists($customer_email_address, $email_addresses_notified)) {
			...
			while (!$products_result->EOF) {
				$products[] = array(
                    ...
                    'model' => $products_result->fields['products_model'] // BMH add model
					);
	...
	$more_than_one = false, $test_mode = false)
{
   ...
   $test_mode_subj = ''; 
	...
	if (file_exists($template_file)) {
		...
		$text_msg_source =  file_get_contents(str_replace($language_folder_path_part, '', $template_file));
