 ...
 */ 
 // BMH   // BMH 2021-10-10 line 195 + 222 correct time format date('Y-m-d H:i:s', time())
 // BMH 2022-08-26  ln 212 time to $t and ln 215 force to strval
...
/* BMH if (!isset($_GET['products_id'])) {    die("No such product"); // should never happen } */
      ...
      if (!$cust_query->EOF) {
		...
		if ($check_notification_subscription->RecordCount() > 0) {
			...
			if (isset($customer_email_address) && $customer_email_address === $email_address) {
                ...
                $t = time();
                //echo '<br> ln212 time = $t ' . $t; echo 'md5 time = '. md5($t); // BMH ** DEBUG
				$subscription_code = substr(md5(strval($t)), 0, 10); // BMH  $subscription_code = substr(md5(time()), 0, 10);
